...
from urllib.request import HTTPBasicAuthHandler, build_opener, urlopen, install_opener
...
class OneTimeSecret(object):
    def __init__(self, username, api_key, api_ver=None, url=None):
        ...
        self.key = api_key
        self.url = "https://%s/api/%s/" % (url, self.api_ver)
    ...
    def retrieve_secret(self, secret_key, passphrase=None):
        ...
        try:
            ...
            url = f"{self.url}/secret/{secret_key}"  # % "secret/%s" % secret_key
    ...
    def status(self):
        """
        Checks server's ability to process our request. Also, sets necessary credentials for
